...
For GitHub Pages, publish the contents of `dist` or serve the repository root directly. The canonical URL is configured as `https://vedasrikavya-blip.github.io/whatsapp-qr-code-gen/`. For Nginx, point the document root to the generated `dist` directory and serve `index.html` as the directory index. Because the shared state is stored after the `#` character, no server rewrite rule is needed for restored configurations.
...
See the root `LICENSE` file for the complete license text. Third-party browser libraries retain their own copyright notices and licenses. `qrcode-generator` and `libphonenumber-js` are open-source projects used through fixed-version browser bundles.
